...
class ReplayMemory(object):
    ...
    def sample(self, batch_size):
        # edited so that sampling won't cause error, "perhaps"
...
class CNN(nn.Module):
    def __init__(self, h, w, outputs):
        ...
        # In original paper, there is no batch normalization and leaky relu.
...
class DQN(object):
    ...
    def select_action(self, state):
        ...
        self.epsilon = self.EPS_END + np.maximum(
            (self.EPS_START - self.EPS_END) * (1 - self.interaction_steps / self.EPS_DECAY), 0)  # linear decay
        if random.random() < self.epsilon:
            ...
            # return torch.tensor([random.sample([0, 1, 2], 1)], device=device,
            #                   dtype=torch.long)  # torch.tensor([[random.randrange(self.action_dim)]], device=device, dtype=torch.long)
        else:
            with torch.no_grad():
                return self.policy_net(state).max(1)[1].view(1, 1)          # .view???
 
    def evaluate_action(self, state, rand=0.1):     # wtf is this. almost 99% same as select_action
    ...
    def update(self):
        ...
        transitions = self.memory.sample(self.BATCH_SIZE)       # transitions = [(sample1_tuple),(sample2_tuple),...]
        batch = Transition(*zip(*transitions))
 
        # *transitions = sample1_tuple , sample2_tuple , ...
        # zip(*transitions) = [[sample1_tuple.state , sample2_tuple.state , ...]
        #                      ,[sample1_tuple.action , sample2_tuple.action , ...]
        #                      ,[sample1_tuple.next_state , ...]
        #                      ,[sample1_tuple.reward , ...]
        #                      ,[sample1_tuple.done , ...]]
        #
        # *zip(*transitions) = [sample1_tuple.state , sample2_tuple.state , ...],
        #                       [sample1_tuple.action , sample2_tuple.action , ...],
        ...
        #                       ([sample1_tuple.state , sample2_tuple.state, ...],
        #                        [sample1_tuple.action , sample2_tuple.action , ...],
        #                        [sample1_tuple.next_state , ...],
        #                        [sample1_tuple.reward , ...],
        #                        [sample1_tuple.done , ...])
        #                       = batch
        #
        # batch.state = [sample1_tuple.state , sample2_tuple.state , ...]
...
class RandomAgent(object):
    ...
    def select_action(self, state):
        self.interaction_steps += 1
        return torch.tensor([random.sample([0, 1, 2], 1)], device=device,
                            dtype=torch.long)  # random.sample([0,0,0,1,1,1,1,1,1,2],1)
...
def main():
    ...
    for i_episode in range(num_episodes):
        ...
        for _ in range(10000):
            ...
            # Here put the transition data on device you want and may convert when update
...
if __name__ == "__main__":
    ...
    # Before you run function, please check hyperparameters again!!
    if args.train:
        main()
    else:
        test()
